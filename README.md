...
- **Multimodal Prototyping Pipeline**: The companion CLI tool (`morphcli`) integrates Gemini 3.1 Pro's advanced vision understanding to vectorize raw PNG/JPEG images directly into clean, animatable SVGs.
...
A robust standalone CLI tool (`morphcli`) is provided inside the `prototypes/` directory. It is designed for debugging complex prompts, massive source SVGs, and even generating net-new SVGs from raw image data (e.g. from output generated by tools like [Mattey Banana](../matteybanana/README.md)) without booting the web server.

```bash
cd prototypes
go build -o morphcli .

# 1. Analyze an SVG for complexity (warns if file is too large for AI processing)
./morphcli analyze ../samples/cat_design.svg

# 2. Ask Gemini 3.1 Pro to intelligently simplify a massive SVG down to < 5KB
./morphcli simplify ../samples/cat_design.svg

# 3. Animate an SVG directly from the terminal
./morphcli animate ../samples/design.svg "make it spin"

# 4. Multimodal Vectorization: Pass a raw PNG/JPEG to Gemini 3.1 Pro to convert it into a crisp, animatable SVG
./morphcli vectorize ../samples/test_raster.png
```
All outputs are saved cleanly to `prototypes/transformed_output.svg` for easy inspection in the browser.
